...
def shoot(i, j, k, utility):
	if j and k:
		...
		if i==1:
			return attack_prob*(stepcost_shoot+finalreward+Y*utility[i-1][j-1][k-1])+(1-attack_prob)*(stepcost+Y*utility[i][j-1][k-1])
		else:
			return attack_prob*(stepcost_shoot+Y*utility[i-1][j-1][k-1])+(1-attack_prob)*(stepcost+Y*utility[i][j-1][k-1])
	else :
		return -100000
...
if os.path.exists("output")==0:
	os.mkdir("output")
solve()
file = open("output/task_1_trace.txt", "w") 
file.write(toprint) 
file.close()
...
iteration=0
solve()
file = open("output/task_2_part_1_trace.txt", "w") 
file.write(toprint) 
file.close()
...
iteration=0
solve()
file = open("output/task_2_part_2_trace.txt", "w") 
file.write(toprint) 
file.close()
...
iteration=0
solve()
file = open("output/task_2_part_3_trace.txt", "w") 
file.write(toprint) 
file.close()
